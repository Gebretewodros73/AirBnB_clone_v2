#!/usr/bin/python3
"""unittest module code for the console"""
...
class TestHBNBCommand(TestCase):
    ...
    def test_fs_create(self):
        """Tests create commands on file storage"""
        with patch('sys.stdout', new=StringIO()) as std:
            ...
            cons_cmd.onecmd('create User name="Divine" age=22 height=6.2')
            tst_id = std.getvalue().strip()
            self.assertIn('User.{}'.format(tst_id), storage.all().keys())
            cons_cmd.onecmd('show User {}'.format(tst_id))
            self.assertIn("'name': 'Divine'", std.getvalue().strip())
            self.assertIn("'age': 22", std.getvalue().strip())
            self.assertIn("'height': 6.2", std.getvalue().strip())
    ...
    def test_db_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as std:
            ...
            cons_cmd.onecmd(
                'create User email="abbahdivine2020@gmail.com" password="12345"')
            tst_id = std.getvalue().strip()
            ...
            self.assertIn('abbahdivine2020@gmail.com', result)
    ...
    def test_db_show(self):
        """Tests the show command with the db storage.
        """
        with patch('sys.stdout', new=StringIO()) as std:
            cons_cmd = HBNBCommand()
            obj = User(email="abbahdivine2020@gmail.com", password="12345")
            db_con = MySQLdb.connect(
                host=getenv('HBNB_MYSQL_HOST'),
                port=3306,
                user=getenv('HBNB_MYSQL_USER'),
            ...
            self.assertIn('abbahdivine2020@gmail.com', result)
            self.assertIn('12345', result)
            self.assertIn('abbahdivine2020@gmail.com', std.getvalue())
